...
export function CarList({
  ...
  const { data: cars, isLoading: carsLoading, error } = useQuery<Car[]>({
      ...
      try {
        ...
        reviews.forEach((review: any) => {
          ...
          reviewsByCarId[carId].avg = 
            (reviewsByCarId[carId].avg * (reviewsByCarId[carId].count - 1) + review.rating) / 
            reviewsByCarId[carId].count;
        });
  ...
  const [favoritedCarIds, setFavoritedCarIds] = useState<Set<number>>(new Set());
  
  useEffect(() => {
    if (favorites && Array.isArray(favorites)) {
      const favIds = new Set(favorites.map((fav: any) => fav.car?.id).filter(Boolean));
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500 mb-4">We couldn't load the car listings. Please try again.</p>
  ...
  if ((!cars || cars.length === 0) && !hideEmptyState) {
    return (
      <div className="text-center py-12 border rounded-lg bg-gray-50">
        ...
        <p className="text-gray-500 mb-4">Try adjusting your search filters or check back later for new listings.</p>
        
        {Object.keys(searchParams).length > 0 && (
          <Button onClick={() => window.location.href = '/cars'} variant="outline" className="mr-2">
            Clear Filters
          </Button>
        )}
